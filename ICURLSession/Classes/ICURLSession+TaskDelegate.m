...
@implementation  ICURLSession (TaskDelegate)
...
totalBytesExpectedToSend:(int64_t)totalBytesExpectedToSend
{
    ...
    if (icSessionTask.progressBlock)
    {
        icSessionTask.progressBlock(totalBytesSent, totalBytesExpectedToSend);
    }
}
...
didCompleteWithError:(nullable NSError *)error
{
    NSString *URLString = task.originalRequest.URL.absoluteString;
    ICURLSessionTask *icSessionTask = [self taskForURLString:URLString];
    icSessionTask.error = error;
    [icSessionTask didFinishReceivedData];
    
    if (error.code != NSURLErrorCancelled)
    {
        if (icSessionTask.error || ([icSessionTask.data length] <= 0 && icSessionTask.dataLocation == nil))
        {
            if (icSessionTask.failBlock)
            {
                icSessionTask.failBlock(icSessionTask.response, icSessionTask.error);
            }
        }
        else if (icSessionTask.successBlock)
        {
            icSessionTask.successBlock(icSessionTask.response, icSessionTask.data);
        }
    }
    
    [self removeTask:icSessionTask inNamespace:icSessionTask.taskNamespace];
 ...
 completionHandler:(void (^)(NSURLSessionResponseDisposition disposition))completionHandler
{
    ...
    ICURLSessionTask *icSessionTask = [self taskForURLString:URLString];
    icSessionTask.response = response;
    ...
    didReceiveData:(NSData *)data
{
    ...
    [icSessionTask appendReceivedData:data];    
}
...
didFinishDownloadingToURL:(NSURL *)location
{
    ...
    ICURLSessionTask *icSessionTask = [self taskForURLString:URLString];
    icSessionTask.dataLocation = location;
    
    if (icSessionTask.downloadFinishBlock)
    {
        icSessionTask.downloadFinishBlock(icSessionTask.response, icSessionTask.dataLocation);
    }
}
...
totalBytesExpectedToWrite:(int64_t)totalBytesExpectedToWrite
{
    ...
    if (icSessionTask.progressBlock)
    {
        icSessionTask.progressBlock(totalBytesWritten, totalBytesExpectedToWrite);
    }
}
